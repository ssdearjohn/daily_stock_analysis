...
class NotificationService:
    ...
    def __init__(self):
        ...
        self._dingtalk_max_bytes = getattr(config, 'dingtalk_max_bytes', 20000)
    ...
    def send_to_dingtalk(self, content: str) -> bool:
        ...
        max_bytes = self._dingtalk_max_bytes  # 从配置读取，默认 20000 字节

        # 检查字节长度，超长则分批发送
        content_bytes = len(formatted_content.encode('utf-8'))
        if content_bytes > max_bytes:
            logger.info(f"钉钉消息内容超长({content_bytes}字节/{len(content)}字符)，将分批发送")
            return self._send_dingtalk_chunked(formatted_content, max_bytes)

        try:
            return self._send_dingtalk_message(formatted_content)
        except Exception as e:
            logger.error(f"发送钉钉消息失败: {e}")
            return False

    def _send_dingtalk_chunked(self, content: str, max_bytes: int) -> bool:
        """
        分批发送长消息到飞书
        ...
        if "\n---\n" in content:
            ...
            # 无法智能分割，按行强制分割
            return self._send_dingtalk_force_chunked(content, max_bytes)
        ...
        logger.info(f"钉钉分批发送：共 {total_chunks} 批")
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                if self._send_dingtalk_message(chunk_with_marker):
                    success_count += 1
                    logger.info(f"钉钉第 {i + 1}/{total_chunks} 批发送成功")
                else:
                    logger.error(f"钉钉第 {i + 1}/{total_chunks} 批发送失败")
            except Exception as e:
                logger.error(f"钉钉第 {i + 1}/{total_chunks} 批发送异常: {e}")
    ...
    def _send_dingtalk_message(self, content: str) -> bool:
        """发送单条钉钉消息（优先使用 Markdown 卡片）"""
        def _post_payload(payload: Dict[str, Any]) -> bool:
            logger.debug(f"钉钉请求 URL: {self._dingtalk_url}")
            logger.debug(f"钉钉请求 payload 长度: {len(content)} 字符")
            ...
            logger.debug(f"钉钉响应内容: {response.text}")
            ...
            if response.status_code == 200:
                ...
                if code == 0:
                    ...
                    logger.error(f"钉钉返回错误 [code={error_code}]: {error_msg}")
    ...
    def _send_dingtalk_force_chunked(self, content: str, max_bytes: int) -> bool:
        """
        强制按字节分割发送（无法智能分割时的 fallback）
        ...
        logger.info(f"钉钉强制分批发送：共 {total_chunks} 批")

        for i, chunk in enumerate(chunks):
            page_marker = f"\n\n📄 ({i + 1}/{total_chunks})" if total_chunks > 1 else ""

            try:
                if self._send_dingtalk_message(chunk + page_marker):
                    success_count += 1
            except Exception as e:
                logger.error(f"钉钉第 {i + 1}/{total_chunks} 批发送异常: {e}")
